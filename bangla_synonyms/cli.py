...
from bangla_synonyms.core import (
    BatchScraper, DatasetManager, DEFAULT_SOURCES, SOURCES,
)
...
def build(
    ...
    scraper = BatchScraper(
        dataset=DatasetManager(), delay=delay, sources=sources, merge=merge,
    )
...
@click.option("--delay", "-d", default=1.0, show_default=True, help="Seconds between requests.")
def get_cmd(words: tuple, offline: bool, sources: tuple, no_merge: bool, delay: float) -> None:
    """
    Look up synonyms for one or more words.
    ...
    Examples:
        ...
        bangla-synonyms get চোখ --no-merge
    """
    sc     = Scrapper(offline=offline, delay=delay, sources=list(sources) or None, merge=not no_merge)
    result = sc.get_many(list(words))
    for word, syns in result.items():
        print(f"{word}: {', '.join(syns) if syns else '—'}")
...
@click.option("--no-merge", "no_merge", is_flag=True, help="Stop at first source with results.")
def build_cmd(limit: int, delay: float, sources: tuple, no_merge: bool) -> None:
    """
    Scrape and build / expand the local dataset.
    ...
    Examples:
        ...
        bangla-synonyms build --sources wiktionary --sources shabdkosh
    """
    added = build(limit=limit, delay=delay, sources=list(sources) or None, merge=not no_merge)
    print(f"[bangla-synonyms] {added} new word(s) added to dataset.")
